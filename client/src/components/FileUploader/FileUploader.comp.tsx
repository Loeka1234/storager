          ...
          if (res.data.fileSizeUploaded)
            setUser(user =>
              user
                ? {
                    ...user,
                    usedStorage: user.usedStorage + res.data.fileSizeUploaded,
                  }
                : user
            );
        })
      ...
      <ModalContent>
        ...
        <ModalBody pb={6}>
          <FileUploaderDropArea
            ...
            disabled={uploadState === "uploading" || uploadState === "finished"}
          />
          <Box w="100%">
            {files.map(file => {
              if (!file) return null;
              ...
              return file ? (
                ...
                >
                  ...
                  {state === "error" ? (
                    <Icon as={BiErrorCircle} color="red" boxSize="32px" />
                  ) : state === "finished" ? (
                    <Icon as={GrFormCheckmark} boxSize="32px" />
                  ) : (
        ...
        <ModalFooter>
          {uploadState === "finished" ? (
            <Button colorScheme="red" mr={3} onClick={closeFileUploader}>
              Close
            </Button>
          ) : (
            <Button
